...
def rotate_point_cloud_and_gt(batch_data,batch_gt,point_nor,z_rotated=True):
    ...
    for k in range(batch_data.shape[0]):
        ...
        point_nor[k, ..., 0:3]   = np.dot(point_nor[k, ..., 0:3].reshape((-1, 3)), rotation_matrix)
            # if batch_gt.shape[-1] > 3:
            #     batch_gt[k, ..., 3:] = np.dot(batch_gt[k, ..., 3:].reshape((-1, 3)), rotation_matrix)

    return batch_data,batch_gt,point_nor
...
def normalize_point_cloud_with_normal(inputs):
    """
    input: pc [N, P, 3]
...
def jitter_point_cloud(data, sigma=0.005, clip=0.03):
    """ Randomly jitter points. jittering is per point.
        Input:
          BxNx3 array, original batch of point clouds
        Return:
